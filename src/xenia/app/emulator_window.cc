...
void EmulatorWindow::StartThreadDumpWatchdog() {
  ...
  std::thread([this]() {
    ...
    while (true) {
      bool pressed = (GetAsyncKeyState(VK_OEM_5) & 0x8000) != 0;  // backslash/pipe key
...
void EmulatorWindow::DumpThreadStates() {
  ...
  for (const auto& thread : threads) {
    ...
    XELOGE("--- {} (Handle: {:08X}, ID: {:08X}) ---",
           name, thread->handle(), thread->thread_id());
    XELOGE("  LR:  {:08X}  CTR: {:08X}", (uint32_t)ctx->lr, (uint32_t)ctx->ctr);
    XELOGE("  r0:  {:08X}  r1(SP): {:08X}  r3: {:08X}  r4: {:08X}",
           (uint32_t)ctx->r[0], (uint32_t)ctx->r[1],
    ...
    XELOGE("  r9:  {:08X}  r10: {:08X}  r11: {:08X}  r12: {:08X}",
           ...
           (uint32_t)ctx->r[11], (uint32_t)ctx->r[12]);
    XELOGE("  r13: {:08X}  r14: {:08X}  r29: {:08X}  r30: {:08X}",
           (uint32_t)ctx->r[13], (uint32_t)ctx->r[14],
           (uint32_t)ctx->r[29], (uint32_t)ctx->r[30]);
    ...
    if (backend) {
      auto* x64_backend =
          dynamic_cast<cpu::backend::x64::X64Backend*>(backend);
      if (x64_backend) {
        auto* bctx =
            x64_backend->BackendContextForGuestContext(ts->context());
        if (bctx && bctx->stackpoints) {
          ...
          if (show > 0) {
            ...
            for (unsigned j = 0; j < show; j++) {
              ...
              XELOGE("    [{}] guest_ret: {:08X}  guest_sp: {:08X}",
                     j, bctx->stackpoints[idx].guest_return_address_,
