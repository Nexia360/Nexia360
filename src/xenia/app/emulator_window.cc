...
constexpr std::string_view kBaseTitle = "Nexia360 - Xbox 360 Emulator";
...
EmulatorWindow::EmulatorWindow(Emulator* emulator,
  ...
  // Create always-visible content browser
  content_browser_ = std::unique_ptr<ContentBrowser>(
      new ContentBrowser(imgui_drawer_.get(), this));
}
...
void EmulatorWindow::OnEmulatorInitialized() {
  ...
  // Start background watchdog for F10 thread dump (works even when UI frozen).
  StartThreadDumpWatchdog();
      ...
      MenuItem::Create(MenuItem::Type::kPopup, "&Network Mode");
  {
...
#else
    // Linux: enumerate interfaces using getifaddrs
    ...
    if (getifaddrs(&ifaddr) == 0) {
      ...
      for (struct ifaddrs* ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next) {
        ...
        std::string label = fmt::format("{} ({})", name, ip_str);

        Network_interfaces_menu->AddChild(MenuItem::Create(
            MenuItem::Type::kString, label, "",
            std::bind(&EmulatorWindow::SetNetworkInterfaceByGUID, this, name)));
      }
      freeifaddrs(ifaddr);
    }
#endif
...
void EmulatorWindow::OnKeyDown(ui::KeyEvent& e) {
  ...
  // Don't process hotkeys when content browser has focus
  if (content_browser_ && content_browser_->is_focused()) {
    return;
  }
...
void EmulatorWindow::DumpThreadStates() {
  auto kernel_state = emulator_->kernel_state();
  if (!kernel_state) {
    XELOGE("F10: No kernel state available");
    return;
  }
  ...
  XELOGE("========== F10 THREAD STATE DUMP ==========");
  XELOGE("Active guest threads: {}", threads.size());

  for (const auto& thread : threads) {
    if (!thread->is_guest_thread()) continue;
    auto* ts = thread->thread_state();
    if (!ts) continue;
    auto* ctx = ts->context();
    if (!ctx) continue;

    std::string name = thread->thread_name();
    if (name.empty()) {
      name = fmt::format("Thread_{:08X}", thread->handle());
    }

    XELOGE("--- {} (Handle: {:08X}, ID: {:08X}) ---", name, thread->handle(),
           thread->thread_id());
    XELOGE("  LR:  {:08X}  CTR: {:08X}", (uint32_t)ctx->lr, (uint32_t)ctx->ctr);
    XELOGE("  r0:  {:08X}  r1(SP): {:08X}  r3: {:08X}  r4: {:08X}",
           (uint32_t)ctx->r[0], (uint32_t)ctx->r[1], (uint32_t)ctx->r[3],
           (uint32_t)ctx->r[4]);
    XELOGE("  r5:  {:08X}  r6:  {:08X}  r7:  {:08X}  r8:  {:08X}",
           (uint32_t)ctx->r[5], (uint32_t)ctx->r[6], (uint32_t)ctx->r[7],
           (uint32_t)ctx->r[8]);
    XELOGE("  r9:  {:08X}  r10: {:08X}  r11: {:08X}  r12: {:08X}",
           (uint32_t)ctx->r[9], (uint32_t)ctx->r[10], (uint32_t)ctx->r[11],
           (uint32_t)ctx->r[12]);
    XELOGE("  r13: {:08X}  r14: {:08X}  r29: {:08X}  r30: {:08X}",
           (uint32_t)ctx->r[13], (uint32_t)ctx->r[14], (uint32_t)ctx->r[29],
           (uint32_t)ctx->r[30]);
    XELOGE("  r31: {:08X}", (uint32_t)ctx->r[31]);

    // Dump guest pseudo-stacktrace from backend context if available.
    auto* backend = emulator_->processor()->backend();
    if (backend) {
      auto* x64_backend = dynamic_cast<cpu::backend::x64::X64Backend*>(backend);
      if (x64_backend) {
        auto* bctx = x64_backend->BackendContextForGuestContext(ts->context());
        if (bctx && bctx->stackpoints) {
          unsigned depth = bctx->current_stackpoint_depth;
          unsigned show = depth < 10 ? depth : 10;
          if (show > 0) {
            XELOGE("  Call stack (most recent first):");
            for (unsigned j = 0; j < show; j++) {
              unsigned idx = depth - 1 - j;
              XELOGE("    [{}] guest_ret: {:08X}  guest_sp: {:08X}", j,
                     bctx->stackpoints[idx].guest_return_address_,
                     bctx->stackpoints[idx].guest_stack_);
            }
          }
        }
      }
...
#else
    // Linux: the guid IS the interface name (e.g. "eth0", "wlan0")
    interface_name = guid;
#endif
...
EmulatorWindow::ControllerHotKey EmulatorWindow::ProcessControllerHotkey(
  ...
  // When content browser exists, block standalone D-PAD and Start —
  // the content browser replaces the old title selector.
  if (content_browser_) {
    bool is_dpad_or_start = (buttons == X_INPUT_GAMEPAD_DPAD_UP ||
