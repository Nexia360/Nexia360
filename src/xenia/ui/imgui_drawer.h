...
class ImGuiDrawer : public WindowInputListener, public UIDrawer {
 public:
  ...
  // === Xenia-Canary compatibility shims ===
  // Canary's netplay UI (xam_ui / gamerpic_browser / create_profile_ui) and the
  // Emulator call these; the Nexia drawer dropped them, so they're mapped back
  // onto this drawer to keep Canary's UI compiling.
  bool IsAnyDialogOpen() const { return !dialogs_.empty(); }
  // Nexia's drawer polls XInput itself (PollXInput); the input system isn't
  // needed - accept and ignore so emulator.cc keeps compiling.
  void LoadInputSystem(hid::InputSystem*) {}
  // The loading-tile icon was a Canary-only embedded asset; return null (ImGui
  // renders nothing for a null texture id). Cosmetic only.
  ImmediateTexture* GetLoadingTileIcon() { return nullptr; }
 ...
 private:
  ...
  // Rising-edge tracking for auto-showing the OSK when a text field activates.
  bool prev_want_text_input_ = false;
