...
void KeyboardDialog::OnDraw(ImGuiIO& io) {
  ...
  if (!ignore_inputs) {
    ...
    if (io.InputQueueCharacters.Size > 0) {
      // The user is typing on a real keyboard, so the on-screen key grid is
      // not what they're driving. Park focus on Done so that Enter/Space (or
      // a stray A press) commits the text instead of activating whichever
      // on-screen key currently holds focus.
      focus_done_button_ = true;
  ...
  if (ImGui::BeginPopupModal(title_.c_str(), &popup_open, flags)) {
    ...
    // Wrap gamepad/keyboard navigation around all edges: moving past the top
    // lands on the bottom (and vice-versa), and moving past the rightmost key
    // of a row lands on the leftmost (and vice-versa).
...
void KeyboardDialog::DrawKeyboardLayout() {
  ...
  if (input_type_ == InputType::kNumber || is_number_mode_) {
  ...
  for (size_t row = 0; row < layout->size(); ++row) {
    ...
    for (size_t col = 0; col < row_keys.size(); ++col) {
      ...
      // Real-keyboard typing parks focus on Done (see focus_done_button_).
      // Takes precedence over the open-focus above; only fires once per burst.
      if (focus_done_button_ && !key.value &&
          std::string(key.label) == "Done") {
        ImGui::SetKeyboardFocusHere();
        focus_done_button_ = false;
        needs_focus_ = false;
      }

      std::string button_id =
          std::string(key.label) + "##" + std::to_string(key_index);
      // Character keys activate on A *press* (PressedOnClick) so typing is
      // immediate. Special keys (Done/Cancel/Shift/<-) keep the default
      // release activation so Done only fires once A is released.
      const ImVec2 key_btn_size(key_size * key.width, key_size);
      bool pressed = key.value
                         ? ImGui::ButtonEx(button_id.c_str(), key_btn_size,
                                           ImGuiButtonFlags_PressedOnClick)
                         : ImGui::Button(button_id.c_str(), key_btn_size);
  ...
  // NOTE: B = Backspace is handled once, on PRESS (with key-repeat), in the
  // direct XInput polling block above. Do NOT also delete here on the ImGui
  // GamepadFaceRight key, or every B press would delete two characters.

  // LB = Numbers only (toggle the numbers-only layout)
  if (ImGui::IsKeyPressed(ImGuiKey_GamepadL1)) {
    is_number_mode_ = !is_number_mode_;
    if (is_number_mode_) {
  ...
  // Left stick click (L3) = Toggle shift
  if (ImGui::IsKeyPressed(ImGuiKey_GamepadL3)) {
